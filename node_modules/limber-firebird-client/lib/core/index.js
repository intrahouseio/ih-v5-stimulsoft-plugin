...
function Transaction(connection) {
    ...
    this.query = function (query, params, queryLimit, callback) {
        ...
        let promise = new Promise((resolve, reject) => {
            ...
            }, {asObject: true, asStream: false, queryLimit: queryLimit});
            return self;
        });
    ...
    this.queryOne = function (query, params, callback) {
    ...
    this.commit = function (callback) {
        let self = this;
        let promise = new Promise((resolve, reject) => {
            self.connection.commit(self, (err) => {
                if (err) {
                    return reject(err);
                } else {
    ...
    this.rollback = function (callback) {
        let self = this;
        let promise = new Promise((resolve, reject) => {
            self.connection.rollback(self, (err) => {
                if (err) {
                    return reject(err);
                } else {
                    return resolve(undefined);
    ...
    this.commitRetaining = function (callback) {
        this.connection.commitRetaining(this, callback);
    };

    this.rollbackRetaining = function (callback) {
...
function Database(connection) {
    ...
    this.__proto__ = Object.create(Events.EventEmitter.prototype, {
        constructor: {
            value: Database,
            enumberable: false
        }
    ...
    this.detach = function (callback, force) {
        let self = this;
        ...
        let promise = new Promise((resolve, reject) => {
            if (!force && self.connection._pending.length > 0) {
                ...
                return resolve(undefined);
            } else if (self.connection._pooled === true) {
                self.emit('detach', () => {
                    return resolve(undefined);
                });
            } else {
    ...
    this.transaction = function (isolation, callback) {

        if (!isolation || isolation instanceof Function) {
            callback = isolation;
            isolation = ISOLATION_READ_COMMITED_NO_WAIT;
        }

        let self = this;
        let promise = new Promise((resolve, reject) => {
            self.startTransaction(isolation, (err, transaction) => {
                if (err) {
                    return reject(err);
                } else {
    ...
    this.startTransaction = function (isolation, callback) {
    ...
    this.executeAux = function (query, params, callback, custom) {
        ...
        self.connection.startTransaction(function (err, transaction) {
            ...
            transaction.executeAux(query, params, function (err, result, meta, isSelect) {
                if (err) {
    ...
    this.sequentially = function (query, params, on, callback, asArray) {
        ...
        let self = this;
        let promise = new Promise((resolve, reject) => {
            if (on === undefined || on instanceof Boolean) {
                reject(new Error('Expected "on" delegate.'));
            } else {
                self.executeAux(query, params, (err) => {
                    if (err) {
    ...
    this.set_user = async function (usuario, descricao) {
        await this.execute('select RESULT from SET_USER_CDE_WIN(?, ?, ?, ?, ?)', [usuario, descricao, null, null, (usuario ? process.env._FB_SET_USER_VERSION : null)]);
    };
}
 ...
 */
exports.attach = async function (options) {
    return new Promise(async (resolve, reject) => {
        ...
        var cnx = this.connection = new Connection(host, port, function (err) {
            ...
            cnx.connect(options, function (err) {
                ...
                let cb = function (err, db) {
                    if (err) return reject(err);
                    return resolve(db);
                };

                if (manager) {
                    cnx.svcattach(options, cb);
                } else {
                    cnx.attach(options, cb);
                }
...
exports.drop = async function (options) {
    let db = await exports.attach(options);
    await db.drop();
};
...
exports.create = async function (options) {
    return new Promise(async (resolve, reject) => {
        ...
        var cnx = this.connection = new Connection(host, port, function (err) {
            var self = cnx;
            if (err) {
                return reject({error: err, message: "Connect error"});
            }
            cnx.connect(options, function (err) {
                if (err) {
...
function ServiceManager(connection) {
    ...
    this._createOutputStream = function (optread, buffersize, callback) {
        var self = this;
        ...
        t.__proto__._read = function () {
            ...
            fct.call(self, {buffersize: buffersize}, function (err, data) {
                if (err) {
                    selfread.push(err.message, this.connection.options.encoding);
                    return;
                }
                if (data.line && data.line.length) {
                    selfread.push(data.line, this.connection.options.encoding);
                } else {
                    selfread.push(null);
                }
            });
    ...
    this._processcapabilities = function (blr, res) {
        var capArray = [
            "WAL_SUPPORT",
    ...
    this._processdbinfo = function (blr, res) {
    ...
    this._processquery = function (buffer, callback) {
    ...
    this.backup = function (options, callback) {
        ...
        blr.addString2(isc_spb_dbname, dbpath, this.connection.options.encoding);
        for (var i = 0; i < bckfiles.length; i++) {
            blr.addString2(isc_spb_bkp_file, bckfiles[i].filename, this.connection.options.encoding);
            // not the end, so we need to write the size of this part (gsplit)
            if (i !== bckfiles.length - 1) {
                blr.addString2(isc_spb_bkp_length, bckfiles[i].sizefile, this.connection.options.encoding);
            }
        }
        if (factor) {
        ...
        var self = this;
        this.connection.svcstart(blr, function (err) {
            if (err) {
    ...
    this.nbackup = function (options, callback) {
        var dbpath = options.database || this.connection.options.filename || this.connection.options.database;
        ...
        blr.addString2(isc_spb_dbname, dbpath, this.connection.options.encoding);
        blr.addString2(isc_spb_nbk_file, bckfile, this.connection.options.encoding);
        blr.addByteInt32(isc_spb_nbk_level, level);
        blr.addString2(isc_spb_nbk_direct, direct, this.connection.options.encoding);
        var opts = 0;
        if (notriggers) {
    ...
    this.restore = function (options, callback) {
        ...
        for (var i = 0; i < bckfiles.length; i++) {
            blr.addString2(isc_spb_bkp_file, bckfiles[i], this.connection.options.encoding);
        }
        blr.addString2(isc_spb_dbname, dbfile, this.connection.options.encoding);
        ...
        if (fixfssdata) {
            blr.addString2(isc_spb_res_fix_fss_data, fixfssdata, this.connection.options.encoding);
        }
        if (fixfssmetadata) {
            blr.addString2(isc_spb_res_fix_fss_metadata, fixfssmetadata, this.connection.options.encoding);
        }
        var opts = 0;
    ...
    this.nrestore = function (options, callback) {
        var bckfiles = options.backupfiles || options.files || null; // format bckfiles ['file1', 'file2', 'file3']
        // for convenience
        if (bckfiles) {
            bckfiles = bckfiles.constructor !== Array ? [bckfiles] : bckfiles;
        ...
        for (var i = 0; i < bckfiles.length; i++) {
            blr.addString2(isc_spb_nbk_file, bckfiles[i], this.connection.options.encoding);
        }
        blr.addString2(isc_spb_dbname, dbpath, this.connection.options.encoding);
        var self = this;
        this.connection.svcstart(blr, function (err) {
...
// only one at time don't use this function directly
    this._fixpropertie = function (options, callback) {
        var dbpath = options.database || this.connection.options.filename || this.connection.options.database;
    ...
    this.commit = function (db, transactid, callback) {
    ...
    this.rollback = function (db, transactid, callback) {
        var dbpath = db || this.connection.options.filename || this.connection.options.database;
        if (dbpath == null || dbpath.length === 0) {
            doError(new Error('No database specified'), callback);
            return;
    ...
    this.recover = function (db, transactid, callback) {
        var dbpath = db || this.connection.options.filename || this.connection.options.database;
        if (dbpath == null || dbpath.length === 0) {
            doError(new Error('No database specified'), callback);
            return;
    ...
    this.getStats = function (options, callback) {
        var dbpath = options.database || this.connection.options.filename || this.connection.options.database;
        ...
        if (objects) {
            blr.addString2(isc_spb_command_line, objects, this.connection.options.encoding);
        }
        var self = this;
        this.connection.svcstart(blr, function (err) {
            if (err) {
    ...
    this.getLog = function (options, callback) {
        var self = this;
        var blr = this.connection._blr;
    ...
    this.getUsers = function (username, callback) {
        ...
        if (username) {
            blr.addString2(isc_spb_sec_username, username, this.connection.options.encoding);
        }
        this.connection.svcstart(blr, function (err) {
            if (err) {
                doError(new Error(err), callback);
    ...
    this.addUser = function (username, password, options, callback) {
        ...
        blr.addString2(isc_spb_sec_username, username, this.connection.options.encoding);
        blr.addString2(isc_spb_sec_password, password, this.connection.options.encoding);
        if (rolename) {
            blr.addString2(isc_dpb_sql_role_name, rolename, this.connection.options.encoding);
        }
        if (groupname) {
            blr.addString2(isc_spb_sec_groupname, groupname, this.connection.options.encoding);
        }
        if (firsname) {
            blr.addString2(isc_spb_sec_firstname, firsname, this.connection.options.encoding);
        }
        if (middlename) {
            blr.addString2(isc_spb_sec_middlename, middlename, this.connection.options.encoding);
        }
        if (lastname) {
            blr.addString2(isc_spb_sec_lastname, lastname, this.connection.options.encoding);
        }
        if (userid != null) {
    ...
    this.editUser = function (username, options, callback) {
        var rolename = options.rolename || null;
        var groupname = options.groupname || null;
        var firsname = options.firstname || null;
        var middlename = options.middlename || null;
    ...
    this.removeUser = function (username, rolename, callback) {
        var blr = this.connection._blr;
        blr.pos = 0;
        blr.addByte(isc_action_svc_delete_user);
        blr.addString2(isc_spb_sec_username, username, this.connection.options);
        if (rolename) {
            blr.addString2(isc_dpb_sql_role_name, rolename, this.connection.options);
        }
    ...
    this.getFbserverInfos = function (infos, options, callback) {
    ...
    this.startTrace = function (options, callback) {
        var self = this;
        var blr = this.connection._blr;
        ...
        blr.addString2(isc_spb_trc_cfg, configfile, this.connection.options.encoding);
        blr.addString2(isc_spb_trc_name, tracename, this.connection.options.encoding);
        this.connection.svcstart(blr, function (err) {
            if (err) {
                doError(new Error(err), callback);
                return;
    ...
    this.suspendTrace = function (options, callback) {
        var self = this;
        var blr = this.connection._blr;
    ...
    this.resumeTrace = function (options, callback) {
        var self = this;
        var blr = this.connection._blr;
        var traceid = options.traceid || null;
    ...
    this.stopTrace = function (options, callback) {
        var self = this;
        var blr = this.connection._blr;
        var traceid = options['traceid'] || null;
    ...
    this.getTraceList = function (options, callback) {
        var self = this;
        var blr = this.connection._blr;
        blr.pos = 0;
    ...
    this.readline = function (options, callback) {
        var buffersize = options.buffersize || 2048;
    ...
    this.readeof = function (options, callback) {
    ...
    this.hasRunningAction = function (options, callback) {
        var buffersize = options.buffersize || 2048;
        var timeout = options.timeout || 60;
        var self = this;
    ...
    this.readusers = function (options, callback) {
        var buffersize = options.buffersize || 2048;
        var timeout = options.timeout || 60;
        var self = this;
    ...
    this.readlimbo = function (options, callback) {
        var buffersize = options.buffersize || 2048;
        var timeout = options.timeout || 60;
        var self = this;
...
function Pool(max, options) {
     ...
     * @returns {Database}
     */
    this.get = async function (user, descricao) {
        var self = this;

        if (self.dbinuse >= self.max) {
            await new Promise(resolve => setTimeout(resolve, 3000));
            throw new Error('EMAXCONN: Too many connections');
        }
        ...
        try {
            if (self.pooldb.length) {
                db = self.pooldb.shift(0);
                await db.set_user(user, descricao);
                return db;
            }

            db = await exports.attach(self.options);
            self.internaldb.push(db);
            db.on('detach', async function (callback) {
            ...
            // fire on server close connection
            db.on('destroy', function () {
                // no in the pool do nothing
                if (self.internaldb.indexOf(db) === -1) {
                    return;
                }
                // if not usable don't put in again in the pool and remove reference on it
                self.internaldb.splice(self.internaldb.indexOf(db), 1);

                // if is in the pool remove also otherwise is in use so --
                if (self.pooldb.indexOf(db) !== -1) {
                    self.pooldb.splice(self.pooldb.indexOf(db), 1);
                } else {
                    self.dbinuse--;
                }
            });
    ...
    this.destroy = async function () {
        var self = this;
        for (let i = this.internaldb.length - 1; i >= 0; i--) {
            const db = this.internaldb[i];
...
var Connection = exports.Connection = function (host, port, callback, options, db) {
    ...
    this._setcachedquery = function (query, statement) {
    ...
    this._bind_events = function (host, port, callback) {
        const self = this;
        self._socket.on('close', function () {
            self._isClosed = true;

            // can't open socket to a server
            if (!self._isOpened) {
                if (callback) {
                    callback(self.error);
                }
            ...
            // socket open no database attached
            if (self._isDetach) {
                while (self._queue.length) {
                    doCallback(undefined, self._queue.shift());
                }
                return;
            }

            // call all queing call with error
            while (self._queue.length) {
                doCallback(new Error('Connection is closed.'), self._queue.shift());
            }

            // for pool management.
            if (!self._isDetach && self.db.connection._pooled) {
                self.db.emit('destroy');
            }
            //
        ...
        self._socket.on('data', function (data) {
            ...
            while (xdr.pos < xdr.buffer.length) {
                ...
                decodeResponse(xdr, cb, self, self.options.lowercase_keys, self.options.encoding, function (err, obj) {
                    ...
                    if (err) {
                        ...
                        if (self.accept?.protocolMinimumType === ptype_lazy_send && self._queue.length > 0 && self._queue[0]) {
    ...
    this._queueEvent = function (callback) {
        var self = this;
    ...
    this.getCachedQuery = function (query) {
        if (this['_cache_query']) {
    ...
    this.connect = function (options, callback) {
        ...
        blr.addString(CNCT_login, options.user, options.encoding);
        blr.addString(CNCT_plugin_name, pluginName, options.encoding);
        blr.addString(CNCT_plugin_list, AUTH_PLUGIN_LIST.join(','), options.encoding);

        if (pluginName === AUTH_PLUGIN_LEGACY) {
            var specificData = crypt.crypt(options.password, LEGACY_AUTH_SALT).substring(2);
            blr.addMultiblockPart(CNCT_specific_data, specificData, options.encoding);
        } else {
            throw new Error('Invalide auth plugin');
        }
        blr.addBytes([CNCT_client_crypt, 4, 0, 0, 0, 0]); // WireCrypt = Disabled
        blr.addString(CNCT_user, os.userInfo().username || 'Unknown', options.encoding);
        blr.addString(CNCT_host, os.hostname(), options.encoding);
        ...
        msg.addString(options.database || options.filename, options.encoding);
    ...
    this.attach = function (options, callback, db) {
        ...
        blr.addString(isc_dpb_lc_ctype, (options.encoding === 'utf8' ? 'UTF8' : 'ISO8859_1'), options.encoding);
        blr.addString(isc_dpb_user_name, user, options.encoding);
        if (this.accept.protocolVersion < PROTOCOL_VERSION13) {
            if (this.accept.protocolVersion === PROTOCOL_VERSION10) {
                blr.addString(isc_dpb_password, password, options.encoding);
            } else {
                blr.addString(isc_dpb_password_enc, crypt.crypt(password, LEGACY_AUTH_SALT).substring(2), options.encoding);
            }
        }
        if (role) {
            blr.addString(isc_dpb_sql_role_name, role, options.encoding);
        }

        blr.addBytes([isc_dpb_process_id, 4]);
        blr.addInt32(process.pid);

        let processName = (process.env._FB_REMOTE_PROCESS || process.title || "").substring(0, 255);

        blr.addString(isc_dpb_process_name, processName, options.encoding);
        if (this.accept.authData) {
            blr.addString(isc_dpb_specific_auth_data, this.accept.authData, options.encoding);
        }

        msg.addInt(op_attach);
        msg.addInt(0);  // Database Object ID
        msg.addString(database, options.encoding);
        msg.addBlr(this._blr);
    ...
    this.detach = function (callback) {
        ...
        msg.addInt(self.dbhandle || 0); // Database Object ID
        msg.addInt(op_disconnect);
    ...
    this.createDatabase = async function (options) {
        return new Promise(async (resolve, reject) => {
            var self = this;

            var database = options.database || options.filename;
            if (database == null || database.length === 0) {
                reject(new Error('No database specified'));
                return;
            }
            ...
            blr.pos = 0;
            blr.addByte(isc_dpb_version1);
            blr.addString(isc_dpb_set_db_charset, (options.encoding === 'utf8' ? 'UTF8' : 'ISO8859_1'), options.encoding);
            blr.addString(isc_dpb_lc_ctype, (options.encoding === 'utf8' ? 'UTF8' : 'ISO8859_1'), options.encoding);
            blr.addString(isc_dpb_user_name, user, options.encoding);
            if (this.accept.protocolVersion < PROTOCOL_VERSION13) {
                if (this.accept.protocolVersion === PROTOCOL_VERSION10) {
    ...
    this.dropDatabase = async function () {
        return new Promise(async (resolve, reject) => {
    ...
    this.throwClosed = function (callback) {
    ...
    this.startTransaction = function (isolation, callback) {
        ...
        blr.addBytes(isolation || ISOLATION_READ_COMMITED_NO_WAIT);
    ...
    this.rollback = function (transaction, callback) {
    ...
    this.commitRetaining = function (transaction, callback) {
    ...
    this.rollbackRetaining = function (transaction, callback) {
    ...
    this.allocateStatement = function (callback) {
    ...
    this.dropStatement = function (statement, callback) {
    ...
    this.closeStatement = function (statement, callback) {
    ...
    this.allocateAndPrepareStatement = function (transaction, query, plan, callback) {
        var self = this;
        var mainCallback = function (err, ret) {
            if (!err) {
                mainCallback.response.handle = ret.handle;
                try {
                    describe(ret.buffer, mainCallback.response, self.options.encoding);
                } catch (e) {
                    if (callback) {
                        callback(e, mainCallback.response);
                        return;
                    } else {
        ...
        msg.addString(query, self.options.encoding);
        msg.addBlr(blr);
    ...
    this.prepare = function (transaction, query, plan, callback) {
        var self = this;

        if (this.accept.protocolVersion === ptype_lazy_send) { // V11 Statement or higher
    ...
    this.prepareStatement = function (transaction, statement, query, plan, callback) {
        ...
        msg.addInt(statement.handle);
        msg.addInt(3); // dialect = 3
        msg.addString(query, this.options.encoding);
        msg.addBlr(blr);
        msg.addInt(65535); // buffer_length
        ...
        this._queueEvent(function (err, ret) {
            ...
            if (!err) {
                describe(ret.buffer, statement, self.options.encoding);
    ...
    this.executeStatement = function (transaction, statement, params, callback) {
            ...
            function putBlobData(index, value, callback) {
                ...
                self.createBlob2(transaction, function (err, blob) {
                    ...
                    if (Buffer.isBuffer(value)) {
                        ...
                        // todo - BLOB_ENCODING
                        b = Buffer.from(value, self.options.encoding);
                    } else if (!isStream) {
                        // todo - BLOB_ENCODING
                        b = Buffer.from(JSON.stringify(value), self.options.encoding);
                    }

                    if (Buffer.isBuffer(b)) {
                        bufferReader(b, BLOB_MAX, function (b, next) {
                    ...
                    value.on('data', function (chunk) {
                        value.pause();
                        isReading = true;
                        bufferReader(chunk, BLOB_MAX, function (b, next) {
                            self.batchSegments(blob, b, next);
                        }, function () {
                            isReading = false;
    ...
    this.sendExecute = function (op, statement, transaction, callback, parameters) {
        var msg = this._msg;
        var blr = this._blr;
        msg.pos = 0;
        blr.pos = 0;
        ...
        if (parameters && parameters.length) {
            CalcBlr(blr, parameters, this.options.encoding);
            ...
            if (this.accept.protocolVersion >= PROTOCOL_VERSION13) {
                ...
                for (i = 0; i < parameters.length; i++) {
                    if (parameters[i].value !== null) {
                        parameters[i].encode(msg, this.options.encoding);
                    }
                }
            } else {
                for (i = 0; i < parameters.length; i++) {
        ...
        if (op === op_execute2) {
            ...
            if (statement.output && statement.output.length) {
                CalcBlr(outputBlr, statement.output, this.options.encoding);
        ...
        if (this.accept.protocolVersion >= PROTOCOL_VERSION16) {
            msg.addInt(0); // sqldata_timeout
        }
    ...
    this.fetch = function (statement, transaction, count, callback) {
        ...
        msg.addInt(op_fetch);
        msg.addInt(statement.handle);
        CalcBlr(blr, statement.output, this.options.encoding);
        msg.addBlr(blr);
        msg.addInt(0); // message number
    ...
    this.fetchAll = function (statement, transaction, callback) {
        var self = this, data = [];
        var loop = function (err, ret) {
            if (err) {
                ...
                // todo - BLOB_ENCODING
                fetch_blob_sync(ret.data, statement.output, true, self.options.encoding, function (err, result) {
                    if (err) {
                        return callback(err);
                    } else {
                        if (statement.custom && statement.custom.queryLimit) {
                            if (((data.length + result.length) * statement.output.length) > statement.custom.queryLimit) {
                                return callback(new Error('queryLimit reached : ' + statement.custom.queryLimit))
                            }
                        }
                        if (statement.custom && statement.custom.asStream && statement.custom.on) {
                            for (var i = 0; i < result.length; i++) {
                                // não passa mais a quantidade total de registros
                                statement.custom.on(result[i]);
                            }
                        } else {
                            data.push(...result);
                        }
    ...
    this.openBlob = function (blob, transaction, callback) {
        var msg = this._msg;
        msg.pos = 0;
    ...
    this.closeBlob = function (blob, callback) {
        var msg = this._msg;
        msg.pos = 0;
    ...
    this.getSegment = function (blob, callback) {
        var msg = this._msg;
        msg.pos = 0;
    ...
    this.createBlob2 = function (transaction, callback) {
        var msg = this._msg;
        msg.pos = 0;
    ...
    this.batchSegments = function (blob, buffer, callback) {
        var msg = this._msg;
        var blr = this._blr;
        msg.pos = 0;
        blr.pos = 0;
    ...
    this.svcattach = function (options, callback, svc) {
        var user = options.user || DEFAULT_USER;
        var password = options.password || DEFAULT_PASSWORD;
        var role = options.role;
        var msg = this._msg;
        ...
        msg.addInt(0);
        msg.addString(DEFAULT_SVC_NAME, options.encoding); // only local for moment
        msg.addBlr(this._blr);
    ...
    this.svcstart = function (spbaction, callback) {
        var msg = this._msg;
        // var blr = this._blr;
        msg.pos = 0;
    ...
    this.svcquery = function (spbquery, resultbuffersize, timeout, callback) {
    ...
    this.svcdetach = function (callback) {
        var self = this;
    ...
    this._timeoutConnection = setTimeout(function () {
        callback(new Error(`Timeout to connect to server: ${host}:${port}`));
        self._timeoutConnection = null;
        if (self._socket) {
    ...
    this._socket = net.createConnection(port, host, () => {
        if (self._timeoutConnection) {
            clearTimeout(self._timeoutConnection);
        } else if (self._socket) {
            self._socket.removeAllListeners();
            self._socket.destroy();
            self._socket = null;
        }
...
function decodeResponse(data, callback, cnx, lowercase_keys, encoding, cb) {
    try {
        do {
        ...
        switch (r) {
            case op_response:
                    ...
                    if (err) {
                        ...
                        if (callback && callback.lazy_count) {
                            ...
                            if (callback.lazy_count > 0) {
                                ...
                                parseOpResponse(data, response, encoding, loop);
                            } else {
                                cb(null, response);
                            }
                        } else {
                ...
                return parseOpResponse(data, response, encoding, loop);
            ...
            case op_sql_response:
                ...
                if (isOpFetch && data.fop) { // could be set when a packet is not complete
                    ...
                    data.fop = false;
                    if (op === op_response) {
                        return parseOpResponse(data, {}, encoding, cb);
                    }
                }
                    ...
                    if (lowerV13) {
                        for (length = output.length; data.fcolumn < length; data.fcolumn++) {
                            ...
                            try {
                                ...
                                let value = item.decode(data, lowerV13, encoding);
                        ...
                        for (length = output.length; data.fcolumn < length; data.fcolumn++) {
                            ...
                            try {
                                ...
                                var value = item.decode(data, lowerV13, encoding);
                                if (item.type === SQL_BLOB) {
                                    value = fetch_blob_async(statement, transaction, value, key, item.subType);
                                }
                                data.frow[key] = value;
                            } catch (e) {
                                // uncomplete packet read
            ...
            case op_accept_data:
                ...
                if (r === op_cond_accept || r === op_accept_data) {
                    var d = data.readString(encoding);
                    accept.pluginName = data.readString(encoding);
                    var is_authenticated = data.readInt();
                    var keys = data.readString(encoding); // keys
...
function parseOpResponse(data, response, encoding, cb) {
    ...
    while (true) {
        ...
        switch (op) {
            ...
            case isc_arg_sql_state:
                if (item.params) {
                    var str = data.readString(encoding);
                    item.params.push(str);
                } else {
                    item.params = [data.readString(encoding)];
                }
...
function describe(buff, statement, encoding) {
    ...
    while (br.pos < br.buffer.length) {
        switch (br.readByteCode()) {
            ...
            case isc_info_sql_get_plan:
                statement.plan = br.readString(encoding);
                break;
                ...
                while (!finishDescribe) {
                    ...
                    switch (bc) {
                        ...
                        case isc_info_sql_field:
                            param.field = br.readString(encoding);
                            break;
                        case isc_info_sql_relation:
                            param.relation = br.readString(encoding);
                            break;
                        case isc_info_sql_owner:
                            param.owner = br.readString(encoding);
                            break;
                        case isc_info_sql_alias:
                            param.alias = br.readString(encoding);
                            break;
                        case isc_info_sql_relation_alias:
                            param.relationAlias = br.readString(encoding);
                            break;
                        case isc_info_truncated:
...
function CalcBlr(blr, xsqlda, encoding) {
...
function fetch_blob_sync(rows, meta, isSelect, encoding, callback) {
    ...
    var parallel = function (group, size) {
        ...
        prows.forEach(function (row) {
            blob_col.forEach(function (col) {
                readblob(row[col], function (err, data, blobSubType) {
                    if (err) {
                        blob_err = err;
                    }
                    nb_blobs--;
                    ...
                    if (blobSubType === 1) {
                        // todo - BLOB_ENCODING
                        row[col] = data.toString(encoding);
                    } else {
                        row[col] = data;
                    }
                    if (nb_blobs === 0) {
    ...
    parallel(0, DEFAULT_FETCHSIZE_BLOB);
}
...
function readblob(blob, callback) {
    ...
    blob(function (err, name, blobSubType, e) {
        if (err) {
            return callback(err);
        }
        var _d = [];
        ...
        e.on('end', function () {
            return callback(null, Buffer.concat(_d, _dlength), blobSubType);
        });
...
function fetch_blob_async(statement, transaction, id, name, blobSubType) {
    if (!id) {
        return null;
    }
    ...
    return function (callback) {
        ...
        statement.connection.openBlob(id, transaction, function (err, blob) {
            ...
            if (err) {
                callback(err, name, blobSubType, e);
                return;
            }
